...
export class EntitySheetHelper {
      ...
      if ( el.classList.contains("attribute-key") ) {
        ...
        for ( let i = 0; i < groups.length; i++ ) {
          if (groups[i].value === val) {
            ui.notifications.error(game.i18n.localize("SRPG.NotifyAttrDuplicate") + ` (${val})`);
            el.value = oldVal;
            attrError = true;
            break;
          }
  ...
  static onAttributeRoll(event) {
    ...
    const shorthand = game.settings.get("srpg", "macroShorthand");
  ...
  static validateGroup(groupName, document) {
    ...
    if ( groups.includes(groupName) ) {
      ui.notifications.error(game.i18n.localize("SRPG.NotifyGroupDuplicate") + ` (${groupName})`);
      return false;
    }

    // Check for group keys that match attribute keys.
    if ( attributes.includes(groupName) ) {
      ui.notifications.error(game.i18n.localize("SRPG.NotifyGroupAttrDuplicate") + ` (${groupName})`);
      return false;
    }

    // Check for reserved group names.
    if ( ["attr", "attributes"].includes(groupName) ) {
      ui.notifications.error(game.i18n.format("SRPG.NotifyGroupReserved", {key: groupName}));
      return false;
    }

    // Check for whitespace or periods.
    if ( groupName.match(/[\s|\.]/i) ) {
      ui.notifications.error(game.i18n.localize("SRPG.NotifyGroupAlphanumeric"));
      return false;
    }
    return true;
  }
  ...
  static async deleteAttributeGroup(event, app) {
    ...
    new Dialog({
      title: game.i18n.localize("SRPG.DeleteGroup"),
      content: `${game.i18n.localize("SRPG.DeleteGroupContent")} <strong>${group.val()}</strong>`,
      buttons: {
        confirm: {
    ...
    return Dialog.prompt({
      ...
      callback: html => {
        ...
        if ( template ) {
          ...
          delete createData.flags.srpg.isTemplate;
        }
